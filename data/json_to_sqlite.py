...
def create_database(db_path):
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS screen_sources (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
...
def insert_screens_data(conn, screens_data):
    ...
    for screen in screens_data:
        ...
        cursor.execute(
            """
            INSERT INTO screens (
                name, city, state, location, width, height, color, plf_format, screen_number,
...
def main():
    ...
    if screens_file_path.exists():
        ...
        print(
            "Warning: Neither 'all_cities_screens.json' nor 'screens.json' found. Screens data will be empty."
        )
