/**
 * Cinema-Specific Templates
 *
 * Uses the generic TemplateEngine from lib/html-templates.js
 * to provide cinema-specific template functions.
 */

try {
    console.log('📦 templates.js loading...');

// Use the globally loaded TemplateEngine from lib/html-templates.js
// Note: Using LegacyTemplateUtils directly for simplicity

// Legacy TemplateUtils interface for backward compatibility
console.log('🔧 Creating LegacyTemplateUtils...');
const LegacyTemplateUtils = {
                ...
                try {
                    if (metric.calculate) {
                        ...
                        const keys = metric.key.split('.').map(key => {
                            // Handle bracket notation like content_support["3d_capability"]
                        ...
                        for (const key of keys) {
                            if (Array.isArray(key)) {
                                // Handle bracket notation [obj, prop]
    ...
    // Visual size comparison template
    ...
    // Sidebar screen details template
    ...
    // Export Legacy TemplateUtils for backward compatibility
    console.log('🔗 Setting window.TemplateUtils from templates.js');
    window.TemplateUtils = LegacyTemplateUtils;
    console.log('✅ window.TemplateUtils set from templates.js, functions available:', Object.keys(LegacyTemplateUtils));
} catch (error) {
    console.error('❌ Fatal error in templates.js:', error);
